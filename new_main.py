...
You are a senior legal advisor specializing in Indian contracts. Below is the complete contract text:

\"\"\"{full_text}\"\"\"

Please produce a concise, layman-friendly summary that a non-lawyer stakeholder can understand easily, but maintain an official and professional tone. Organize it under these headings:

1. Parties & Roles:  
   Who is involved, and what each party will do.

2. Duration & Renewal:  
   How long the contract lasts and what happens at the end.

3. Main Responsibilities:  
   In plain English, what each side must do (e.g., manufacturing, payments, deliveries).

4. Pricing & Payments:  
   How money changes hands—deposit, balance, shipping costs.

5. Termination & Dispute Resolution:  
   Under what circumstances the contract can end, and how disputes are settled.

6. Key Legal Concerns (India-Focused):  
   Briefly point out any vague language, one-sided obligations, or potential issues under Indian law.

Use short sentences, avoid legal jargon, and keep each bullet to one or two lines.  
"""
    ...
    for idx, chunk in enumerate(chunks):
        ...
        prompt = f"""
        You are a legal analyst with expertise in Indian contract law. Analyze the following chunk:
...
You are a senior legal advisor familiar with Indian law. Below is a clause-by-clause analysis of a contract:
...
- Legal Risks or Unclear Clauses (with reference to Indian statutes or norms)
- Suggestions for Legal Review (including potential fixes or redlines)
"""
        try:
...
if __name__ == "__main__":
    ...
    generate_pdf_report(
        ...
        output_pdf_path="contract_two_report.pdf"
    )
